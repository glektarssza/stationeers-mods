...
Code must adhere to the style rules set out and enforced by the CSpell and
`pre-commit` tools.

You should run `npx cspell lint .` and `pre-commit run --all-files` prior to
submitting pull requests. Any pull requests which fail linting will be rejected
until they pass.

Alternatively you can run the VSCode command `workbench.action.tasks.runTask`
and then run the `Lint` task to automatically run the above commands inside the
VSCode IDE.
